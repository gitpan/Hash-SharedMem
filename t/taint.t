...
foreach(
	...
	$sh = $_->();
	is_unt { shash_getd($sh, "a0") } !!1;
	is_mbt { shash_getd($sh, tainted("a0")) } !!1;
	is_mbt { [ substr($^X, 0, 0), shash_getd($sh, "a0") ]->[1] } !!1;
	is_unt { shash_getd($sh, "a20") } !!1;
	is_mbt { shash_getd($sh, tainted("a20")) } !!1;
	is_mbt { [ substr($^X, 0, 0), shash_getd($sh, "a20") ]->[1] } !!1;
	is_unt { shash_getd($sh, "c0") } !!0;
	is_mbt { shash_getd($sh, tainted("c0")) } !!0;
	is_mbt { [ substr($^X, 0, 0), shash_getd($sh, "c0") ]->[1] } !!0;
	is_tnt { shash_get($sh, "a1") } "b1";
	is_tnt { shash_get($sh, tainted("a1")) } "b1";
	is_tnt { [ substr($^X, 0, 0), shash_get($sh, "a1") ]->[1] } "b1";
	is_tnt { shash_get($sh, "a20") } $a20;
	is_tnt { shash_get($sh, tainted("a20")) } $a20;
	is_tnt { [ substr($^X, 0, 0), shash_get($sh, "a20") ]->[1] } $a20;
	is_unt { shash_get($sh, "c1") } undef;
	is_mbt { shash_get($sh, tainted("c1")) } undef;
	is_mbt { [ substr($^X, 0, 0), shash_get($sh, "c1") ]->[1] } undef;
}
